document.addEventListener("DOMContentLoaded", () => {
  ...
  const drawWheel = () => {
    ...
    for (let i = 0; i < numOptions; i++) {
      ...
      ctx.fillStyle = i % 2 === 0 ? "#0022c9" : "#d32525";
      ctx.fill();
      ctx.strokeStyle = "#fff";
      ctx.stroke();
  ...
  const determineResult = () => {
    ...
    // Calculate the effective angle: how much the pointer is offset relative to the wheel's rotation
    let effectiveAngle = (pointerAngle - currentAngle) % (Math.PI * 2);
    if (effectiveAngle < 0) effectiveAngle += Math.PI * 2;

    // Determine the index based on the effective angle
    const index = Math.floor(effectiveAngle / arc) % numOptions;
